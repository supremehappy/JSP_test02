...
public class BBSListServlet extends HttpServlet {
    ...
    private void readBBS(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException{//메뉴를 눌렀을 때의 처리
		...
		int pageNum = readPageNum1();//페이지 갯수를 찾음
		bbsList.setPageNum(pageNum);
		//화면을 게시글 출력 JSP로 바꾸면서 list를 전달한다.
		request.setAttribute("BBS_LIST", bbsList);
		RequestDispatcher rd = request.getRequestDispatcher(
				"index.jsp?BODY=bbsListView.jsp");
		rd.forward(request, response);
    }
    private void readNextPage1(String lastSeqNo, HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException{
    	BBSList list=new BBSList();
    	CrudHome crud = new CrudHome();
    	List result = crud.selectNextPage1(Integer.parseInt(lastSeqNo));
    ...
    private void readPrevPage1(String firstSeqNo, HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException{
    	BBSList list = new BBSList(); 
    	CrudHome crud = new CrudHome();
    	List result = crud.selectPrevPage1(Integer.parseInt(firstSeqNo));
    	Iterator it = result.iterator();
    	for(int i=0; i<5; i++){
    		if(it.hasNext()){
    			BBSItem item = (BBSItem)it.next();
    	...
    	int cnt = crud.selectPrevCount1(Integer.parseInt(firstSeqNo));
    ...
    private void readPage1(String pageNo, HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException{
    	BBSList list = new BBSList();
    	CrudHome crud = new CrudHome();
    	...
    	int cnt = crud.selectNextCount1(Integer.parseInt(pageNo));
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(pageNo != null){
			readPage1(pageNo, request, response);
		}else if(lastSeqNo != null){//다음 페이지 링크를 누른경우
			readNextPage1(lastSeqNo, request, response);
		}else if(firstSeqNo != null){//이전 페이지 링크를 누른경우
			readPrevPage1(firstSeqNo, request, response);
		}else{//게시판 보기 메뉴를 누른 경우
			readBBS(request, response);
		}
	}
	private int readPageNum1(){//페이지 갯수를 리턴 메서드
		int pageNum = 0; Connection con=null;Statement stmt=null;
		CrudHome crud = new CrudHome();
		pageNum = crud.selectPage1();
		return (pageNum+4) / 5;
	}
